  ...
  try {
    ...
    */
    ...
    var addresses = {
      ...
      stakingTokenStakedERC20: {
        '0x5fD8E3112676f69a3613c19597778E6f7A902d7C': new BigNumber('2500').mul(
          WeiPerEth
        ), //fish-to-uni
        '0xB1637bE0173330664adecB343faF112Ca837dA06': new BigNumber('2500').mul(
          WeiPerEth
        ), //fish-to-weth
        '0x6109443fcaf515a23f30248eba1e5ebeb7f53c55': new BigNumber('2500').mul(
          WeiPerEth
        ), //fish-to-tori
        '0xb25e6db21929badf86c6711367d5bd0ea622f42d': new BigNumber('2500').mul(
          WeiPerEth
        ), //fish-to-usdc
      },
    }
    ...
    */
    ...
    async function getContractFactory(contractName) {
      const fileName = 'browser/contracts/artifacts/'.concat(
        contractName,
        '.json'
      )
      console.log('getContractFactory retrieving file ', fileName)
      const metadata = JSON.parse(
        await remix.call('fileManager', 'getFile', fileName)
      )
    ...
    */
    ...
    if (
      (await stakingFactory.signer.getAddress()) !== (await signer.getAddress())
    ) {
    ...
    console.log(
      'Deploying token for genesis ',
      _liveDate,
      ' using reward token ',
      addresses.stakingTokenReward
    )

    var factoryContract = await stakingFactory.deploy(
      addresses.stakingTokenReward,
      _liveDate
    )
    ...
    for (var address of theAddresses) {
      const val = addresses.stakingTokenStakedERC20[address]
      console.log(
        'Deploying staking contract for ',
        address,
        ' for amount ',
        val.toString()
      )
      ...
      if (confirmations.status == 1) {
        console.log(
          'Successfully deployed staking contract for staking token ',
          address
        )
      } else {
        console.log(
          'Waited 3 confirmations but failed to deploy for staking token ',
          address
        )
      }
    }
