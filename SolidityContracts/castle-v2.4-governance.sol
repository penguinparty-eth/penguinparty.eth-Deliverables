...
contract CASTLE is IERC20,Context,Ownable {
    ...
    function wrap(uint256 amount) public returns (uint256) {
        ...
        _writeCheckpoint(msg.sender, numCheckpoints[msg.sender], srcRepOld, _balances[msg.sender]);
        }
        else {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint32 srcRepNum = numCheckpoints[msg.sender];
        uint32 srcRepNumFees = numCheckpoints[_feeTarget0];
        uint256 srcRepOld = srcRepNum > 0 ? checkpoints[msg.sender][srcRepNum - 1].votes : 0;
        uint256 srcRepOldFees = srcRepNum > 0 ? checkpoints[_feeTarget0][srcRepNum - 1].votes : 0;
        _writeCheckpoint(msg.sender, srcRepNum, srcRepOld, _balances[msg.sender]);
        _writeCheckpoint(_feeTarget0, srcRepNumFees,srcRepOldFees,_balances[_feeTarget0]);
        }
