...
contract commonwealthCredit is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    uint256 private _totalSupply;
    ...
    function setAtokenAddress(address atoken) onlyOwner public returns(address) {
        Atokenaddx = AToken(atoken);
        _atoken = atoken;
        emit Change(atoken,"atoken");
        return _atoken;
    }
    function setWtokenAddress(address wtoken) onlyOwner public returns(address) {
        _wtoken = wtoken;
        emit Change(wtoken,"wtoken");
        return _wtoken;
    }
    function wrap(uint256 amount) public returns (uint256) {
        AToken token = AToken(_atoken);
        require(token.transferFrom(msg.sender, _wtoken, amount),"Not enough tokens!");
